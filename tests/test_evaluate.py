...
def test_create_results_dir_default(tmp_path):
    """It should default to <path>/results/[timestamp]_<name>/"""
    bench_path = tmp_path / "benchmarks" / "custom-bench"
    bench_path.mkdir(parents=True)

    outdir = create_results_dir(bench_path, None, "custom-bench")
